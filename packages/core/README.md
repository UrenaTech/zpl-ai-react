...
Shared ZPL.AI barcode and ZPL rendering runtime for browser clients. React and Angular packages use this package for requests, retries, errors, barcode types, and PNG cropping.
...
`renderBarcode` sends `{ type, data }` to `POST https://app.zpl.ai/api/public/barcode/img` and returns the original PNG, including any API-provided footer and whitespace. `renderZpl` sends ZPL to `POST https://app.zpl.ai/api/public/zpl/img` with 4 × 2 inch dimensions, 8 dpmm, a white background, and empty parameters. Use `cropPng` only when you want the browser to crop the label to its non-white content: it requires `document`, Canvas 2D, and `createImageBitmap`. Both request functions require an `AbortSignal` and use `fetch` with `cache: "no-store"`.
...
This client requires the ZPL.AI public endpoints and CORS access from your origin. Browser API keys are visible to users: never put a secret key in client-side code or a public environment variable. Generate public keys in [ZPL.AI settings](https://app.zpl.ai/settings/apikeys).
